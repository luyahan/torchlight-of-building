...
if (speedAffix) {
  ...
  console.log(`  Range: ${speedAffix.valueRanges[0].min}-${speedAffix.valueRanges[0].max}%`);
...
console.log(`  Found ${cooldownAffixes.length} cooldown affixes (showing first 3):`);
...
console.log(`  Equipment types: ${new Set(ALL_GEAR_AFFIXES.map(a => a.equipmentType)).size}`);
console.log(`  Affix types: ${new Set(ALL_GEAR_AFFIXES.map(a => a.affixType)).size}`);

const withRanges = ALL_GEAR_AFFIXES.filter(a => a.valueRanges.length > 0).length;
const multiEffect = ALL_GEAR_AFFIXES.filter(a => a.template.includes("\n")).length;
