import { BaseGearAffix } from "./types";

export const TWO_HANDED_AXE_SUFFIX_AFFIXES = [
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "{0}% additional damage applied to Life",
    "valueRanges": [
      {
        "min": 61,
        "max": 78
      }
    ],
    "rawAffix": "`(61-78)`% additional damage applied to Life"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "+{0}% additional damage against Frozen enemies",
    "valueRanges": [
      {
        "min": 106,
        "max": 137
      }
    ],
    "rawAffix": "`+(106-137)`% additional damage against Frozen enemies"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "+{0}% Combo Damage Enhancement if the Combo Finisher cast recently consumes at least 8Combo Point(s)",
    "valueRanges": [
      {
        "min": 110,
        "max": 130
      }
    ],
    "rawAffix": "`+(110-130)`% Combo Damage Enhancement if the Combo Finisher cast recently consumes at least `8`Combo Point(s)"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "+{0}% Demolisher Charge Restoration Speed",
    "valueRanges": [
      {
        "min": 251,
        "max": 325
      }
    ],
    "rawAffix": "`+(251-325)`% Demolisher Charge Restoration Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "+{0}% Focus Damage Enhancement",
    "valueRanges": [
      {
        "min": 96,
        "max": 120
      }
    ],
    "rawAffix": "`+(96-120)`% Focus Damage Enhancement"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "+60% Steep Strike chance.\n +{0}% additional Steep Strike Damage",
    "valueRanges": [
      {
        "min": 49,
        "max": 62
      }
    ],
    "rawAffix": "`+60`% Steep Strike chance.<> `+(49-62)`% additional Steep Strike Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "0",
    "template": "Adds 90% of the Main-Hand Weapon's damage to Base Trauma Damage",
    "valueRanges": [],
    "rawAffix": "Adds `90`% of the Main-Hand Weapon's damage to Base Trauma Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "{0}% additional damage applied to Life",
    "valueRanges": [
      {
        "min": 43,
        "max": 60
      }
    ],
    "rawAffix": "`(43-60)`% additional damage applied to Life"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "+{0}% Demolisher Charge Restoration Speed",
    "valueRanges": [
      {
        "min": 176,
        "max": 250
      }
    ],
    "rawAffix": "`+(176-250)`% Demolisher Charge Restoration Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "+{0}% Focus Damage Enhancement",
    "valueRanges": [
      {
        "min": 70,
        "max": 90
      }
    ],
    "rawAffix": "`+(70-90)`% Focus Damage Enhancement"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "+{0}% additional damage against Frozen enemies",
    "valueRanges": [
      {
        "min": 75,
        "max": 105
      }
    ],
    "rawAffix": "`+(75-105)`% additional damage against Frozen enemies"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "+{0}% Combo Damage Enhancement if the Combo Finisher cast recently consumes at least 8Combo Point(s)",
    "valueRanges": [
      {
        "min": 80,
        "max": 100
      }
    ],
    "rawAffix": "`+(80-100)`% Combo Damage Enhancement if the Combo Finisher cast recently consumes at least `8`Combo Point(s)"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "+46% Steep Strike chance.\n +{0}% additional Steep Strike Damage",
    "valueRanges": [
      {
        "min": 35,
        "max": 48
      }
    ],
    "rawAffix": "`+46`% Steep Strike chance.<> `+(35-48)`% additional Steep Strike Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Ultimate",
    "tier": "1",
    "template": "Adds 77% of the Main-Hand Weapon's damage to Base Trauma Damage",
    "valueRanges": [],
    "rawAffix": "Adds `77`% of the Main-Hand Weapon's damage to Base Trauma Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Focus Speed",
    "valueRanges": [
      {
        "min": 120,
        "max": 140
      }
    ],
    "rawAffix": "`+(120-140)`% Focus Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Affliction Effect",
    "valueRanges": [
      {
        "min": 151,
        "max": 195
      }
    ],
    "rawAffix": "`+(151-195)`% Affliction Effect"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Critical Strike Damage",
    "valueRanges": [
      {
        "min": 181,
        "max": 234
      }
    ],
    "rawAffix": "`+(181-234)`% Critical Strike Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Critical Strike Rating",
    "valueRanges": [
      {
        "min": 328,
        "max": 468
      }
    ],
    "rawAffix": "`+(328-468)`% Critical Strike Rating"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Attack Critical Strike Rating for this gear",
    "valueRanges": [
      {
        "min": 51,
        "max": 65
      }
    ],
    "rawAffix": "`+(51-65)`% Attack Critical Strike Rating for this gear"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "0",
    "template": "+{0}% Attack and Cast Speed\n +{1}% Minion Attack and Cast Speed",
    "valueRanges": [
      {
        "min": 66,
        "max": 94
      },
      {
        "min": 66,
        "max": 94
      }
    ],
    "rawAffix": "`+(66-94)`% Attack and Cast Speed<> `+(66-94)`% Minion Attack and Cast Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Affliction Effect",
    "valueRanges": [
      {
        "min": 106,
        "max": 150
      }
    ],
    "rawAffix": "`+(106-150)`% Affliction Effect"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Critical Strike Damage",
    "valueRanges": [
      {
        "min": 127,
        "max": 180
      }
    ],
    "rawAffix": "`+(127-180)`% Critical Strike Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Critical Strike Rating",
    "valueRanges": [
      {
        "min": 252,
        "max": 360
      }
    ],
    "rawAffix": "`+(252-360)`% Critical Strike Rating"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Attack Critical Strike Rating for this gear",
    "valueRanges": [
      {
        "min": 36,
        "max": 50
      }
    ],
    "rawAffix": "`+(36-50)`% Attack Critical Strike Rating for this gear"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Attack and Cast Speed\n +{1}% Minion Attack and Cast Speed",
    "valueRanges": [
      {
        "min": 51,
        "max": 72
      },
      {
        "min": 51,
        "max": 72
      }
    ],
    "rawAffix": "`+(51-72)`% Attack and Cast Speed<> `+(51-72)`% Minion Attack and Cast Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Advanced",
    "tier": "1",
    "template": "+{0}% Focus Speed",
    "valueRanges": [
      {
        "min": 90,
        "max": 110
      }
    ],
    "rawAffix": "`+(90-110)`% Focus Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% Elemental Damage",
    "valueRanges": [
      {
        "min": 217,
        "max": 281
      }
    ],
    "rawAffix": "`+(217-281)`% Elemental Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% Erosion Damage",
    "valueRanges": [
      {
        "min": 217,
        "max": 281
      }
    ],
    "rawAffix": "`+(217-281)`% Erosion Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% Physical Damage",
    "valueRanges": [
      {
        "min": 217,
        "max": 281
      }
    ],
    "rawAffix": "`+(217-281)`% Physical Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% Energy Shield Regain",
    "valueRanges": [
      {
        "min": 31,
        "max": 39
      }
    ],
    "rawAffix": "`+(31-39)`% Energy Shield Regain"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% Life Regain",
    "valueRanges": [
      {
        "min": 31,
        "max": 39
      }
    ],
    "rawAffix": "`+(31-39)`% Life Regain"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "0",
    "template": "+{0}% gear Attack Speed",
    "valueRanges": [
      {
        "min": 33,
        "max": 42
      }
    ],
    "rawAffix": "`+(33-42)`% gear Attack Speed"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% Elemental Damage",
    "valueRanges": [
      {
        "min": 152,
        "max": 216
      }
    ],
    "rawAffix": "`+(152-216)`% Elemental Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% Erosion Damage",
    "valueRanges": [
      {
        "min": 152,
        "max": 216
      }
    ],
    "rawAffix": "`+(152-216)`% Erosion Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% Physical Damage",
    "valueRanges": [
      {
        "min": 152,
        "max": 216
      }
    ],
    "rawAffix": "`+(152-216)`% Physical Damage"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% Energy Shield Regain",
    "valueRanges": [
      {
        "min": 22,
        "max": 30
      }
    ],
    "rawAffix": "`+(22-30)`% Energy Shield Regain"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% Life Regain",
    "valueRanges": [
      {
        "min": 22,
        "max": 30
      }
    ],
    "rawAffix": "`+(22-30)`% Life Regain"
  },
  {
    "equipmentTypeKey": "two_handed_axe",
    "equipmentSlot": "Two-Handed",
    "equipmentType": "Two-Handed Axe",
    "affixType": "Suffix",
    "craftingPool": "Basic",
    "tier": "1",
    "template": "+{0}% gear Attack Speed",
    "valueRanges": [
      {
        "min": 23,
        "max": 32
      }
    ],
    "rawAffix": "`+(23-32)`% gear Attack Speed"
  }
] as const satisfies readonly BaseGearAffix[];

export type TwoHandedAxeSuffixAffix = (typeof TWO_HANDED_AXE_SUFFIX_AFFIXES)[number];
